    ...
    <div className="flex items-end gap-[3px] h-12">
      {data.map((v, i) => (
        <div key={i} className="flex flex-col items-center gap-[3px]">
          <div style={{
            width: 18, height: Math.max(4, (v / max) * 42),
...
function StackedChart({ campaign }: { campaign: Campaign }) {
  ...
  const totals = Array.from({ length: maxDays }, (_, i) =>
    platforms.reduce((sum, p) => sum + allDaily[p][i], 0)
  );
  const maxTotal = Math.max(...totals, 1);
  const W = 520, H = 160, PAD = 30;
  const chartW = W - PAD * 2, chartH = H - PAD - 10;

  const getPoints = (platIdx: number) =>
    Array.from({ length: maxDays }, (_, i) => {
      const x = PAD + (i / Math.max(maxDays - 1, 1)) * chartW;
      let stack = 0;
      for (let p = 0; p <= platIdx; p++) stack += allDaily[platforms[p]][i];
      const y = H - PAD - (stack / maxTotal) * chartH;
      return `${x},${y}`;
    });

  const getBase = (platIdx: number) =>
    Array.from({ length: maxDays }, (_, i) => {
      const x = PAD + (i / Math.max(maxDays - 1, 1)) * chartW;
      let stack = 0;
      for (let p = 0; p < platIdx; p++) stack += allDaily[platforms[p]][i];
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full max-w-[520px]">
      {[0.25, 0.5, 0.75, 1].map(f => (
        <g key={f}>
          <line x1={PAD} x2={W - PAD} y1={H - PAD - f * chartH} y2={H - PAD - f * chartH}
            stroke="var(--border)" strokeWidth={0.5} strokeDasharray="3,3" />
          <text x={PAD - 4} y={H - PAD - f * chartH + 3} textAnchor="end"
            className="text-[9px] font-mono" fill="var(--muted)">
            {formatNum(Math.round(maxTotal * f))}
          </text>
        </g>
      ))}
      {DAYS.map((d, i) => (
        <text key={d} x={PAD + (i / Math.max(maxDays - 1, 1)) * chartW} y={H - 6} textAnchor="middle"
          className="text-[9px] font-mono" fill="var(--muted)">{d}</text>
      ))}
      {[...platforms].reverse().map(p => {
        const platIdx = platforms.indexOf(p);
        const top = getPoints(platIdx);
        const base = getBase(platIdx);
        const d = `M${top.join(' L')} L${[...base].reverse().join(' L')} Z`;
        return <path key={p} d={d} fill={PLATFORM_CONFIG[p].color} opacity={0.25} />;
      })}
      {platforms.map((p, i) => (
        <polyline key={p} points={getPoints(i).join(' ')} fill="none" stroke={PLATFORM_CONFIG[p].color} strokeWidth={2} />
      ))}
    </svg>
  );
}
...
function ROIBadge({ cpv }: { cpv: number }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wider font-mono"
...
export default function Dashboard() {
  ...
  const leaderboard = useMemo(() => {
    return creators.map(c => ({ ...c, stats: getCreatorStats(c) }))
      .sort((a, b) => a.stats.cpv - b.stats.cpv);
  }, [creators]);
  ...
  const activeCreator = selectedCreatorId ? creators.find(c => c.id === selectedCreatorId) : null;
  const activeCampaign = activeCreator
    ? (selectedCampaignId
        ? activeCreator.campaigns.find(c => c.id === selectedCampaignId)
        : activeCreator.campaigns[0]) || null
    : null;
  ...
  const handleAddCampaign = async (data: any) => {
    ...
    try {
      const res = await fetch('/api/campaigns', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...data, creator_id: activeCreator.id }),
      });
      if (res.ok) fetchCreators();
    } catch (e) { console.error(e); }
  };
  ...
  const handleDeleteCreator = async (id: string) => {
    ...
    try {
      ...
      fetchCreators();
      if (selectedCreatorId === id) { setView('overview'); setSelectedCreatorId(null); }
    } catch (e) { console.error(e); }
  };
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--bg)' }}>
      {/* Header */}
      <header className="px-7 py-5 flex items-center justify-between sticky top-0 z-50"
        style={{ borderBottom: '1px solid var(--border)', background: 'linear-gradient(180deg, #12121a 0%, transparent 100%)', backdropFilter: 'blur(20px)' }}>
        <div className="flex items-center gap-3.5">
          <div className="w-9 h-9 rounded-[10px] flex items-center justify-center text-lg font-extrabold text-white"
            style={{ background: 'linear-gradient(135deg, #ff2d55, #c837ab)' }}>R</div>
          <div>
        ...
        <div className="flex gap-2.5">
          {view === 'creator' && (
            <button onClick={() => { setView('overview'); setSelectedCreatorId(null); setSelectedCampaignId(null); }}
      ...
      <div className="px-7 py-6 max-w-[1280px] mx-auto">
        ...
        {!loading && creators.length === 0 && view === 'overview' && (
          <div className="animate-fade-up text-center py-24 rounded-2xl"
            style={{ background: 'var(--card)', border: '1px solid var(--border)' }}>
            <div className="text-5xl mb-4">🎮</div>
            <div className="text-xl font-bold mb-2">No creators yet</div>
            <div className="text-sm mb-6" style={{ color: 'var(--muted)' }}>Add your first Roblox content creator to start tracking campaign performance</div>
            <button onClick={() => setShowAddCreator(true)}
              className="px-6 py-3 rounded-lg text-sm font-bold cursor-pointer border-none text-white"
              style={{ background: 'linear-gradient(135deg, #ff2d55, #c837ab)' }}>
        ...
        {!loading && creators.length > 0 && view === 'overview' && (
          <>
            ...
            <div className="grid grid-cols-4 gap-3.5 mb-7">
              {[
                { label: 'TOTAL VIEWS', value: formatNum(globalStats.views), sub: 'across all campaigns' },
                ...
                { label: 'CREATORS', value: creators.length.toString(), sub: 'active in roster' },
              ].map((s, i) => (
                <div key={i} className="animate-fade-up rounded-[14px] p-5"
                  style={{ background: 'var(--card)', border: '1px solid var(--border)', animationDelay: `${i * 0.06}s` }}>
            ...
            {/* Platform Split */}
            <div className="animate-fade-up rounded-[14px] p-5 mb-7"
              style={{ background: 'var(--card)', border: '1px solid var(--border)', animationDelay: '0.25s' }}>
                ...
                {(Object.entries(PLATFORM_CONFIG) as [PlatformKey, typeof PLATFORM_CONFIG[PlatformKey]][]).map(([key, cfg]) => {
                  ...
                  return (
                    <div key={key} className="flex-1">
                      ...
                      <div className="text-[10px] font-mono mt-1" style={{ color: 'var(--muted)' }}>{pct.toFixed(1)}% of total</div>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="animate-fade-up" style={{ animationDelay: '0.35s' }}>
              <div className="flex items-center justify-between mb-4">
                <div className="text-[10px] font-bold tracking-[1.5px] font-mono" style={{ color: 'var(--muted)' }}>
                  CREATOR LEADERBOARD — RANKED BY ROI
                </div>
              ...
              <div className="flex flex-col gap-2.5">
                {leaderboard.map((c, i) => {
                  ...
                  return (
                    <div key={c.id}
                      onClick={() => { setSelectedCreatorId(c.id); setSelectedCampaignId(null); setView('creator'); }}
                      ...
                      <div className="w-7 h-7 rounded-[7px] flex items-center justify-center text-[13px] font-bold font-mono"
                        style={{ background: 'var(--surface)', color: 'var(--muted)' }}>{i + 1}</div>
                      ...
                      <div className="flex gap-5 items-center">
                        ...
                        {platformFilter !== 'all' && (
                          <div className="text-right">
                            <div className="text-[10px] font-semibold mb-0.5" style={{ color: PLATFORM_CONFIG[platformFilter].color }}>
                              {PLATFORM_CONFIG[platformFilter].label}
                            </div>
                            <div className="text-sm font-bold font-mono">{formatNum(views)}</div>
                          </div>
                        )}
        ...
        {view === 'creator' && activeCreator && (
          <>
            {/* Profile Card */}
            <div className="animate-fade-up rounded-2xl p-6 mb-6 flex items-center gap-5"
              style={{ background: 'var(--card)', border: '1px solid var(--border)' }}>
                  ...
                  {(Object.keys(PLATFORM_CONFIG) as PlatformKey[]).map(p => {
                    ...
                    return (
                      <a key={p} href={link || '#'} target="_blank" rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 px-3 py-1 rounded-[7px] text-xs font-semibold font-mono no-underline transition-all"
                        style={{ background: cfg.bg, color: cfg.color, border: `1px solid ${cfg.color}22` }}>
                        {cfg.icon} {username ? `@${username}` : 'linked ✓'}
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowAddCampaign(true)}
                  className="px-4 py-2.5 rounded-lg text-xs font-bold cursor-pointer"
                  style={{ border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text)' }}>
                  + Log Campaign
                </button>
                <button onClick={() => handleDeleteCreator(activeCreator.id)}
                  className="px-3 py-2.5 rounded-lg text-xs cursor-pointer"
                  style={{ border: '1px solid rgba(255,82,82,0.2)', background: 'rgba(255,82,82,0.05)', color: '#ff5252' }}>
                  ✕
                </button>
              </div>
            </div>
            ...
            {activeCreator.campaigns.length > 0 && (
              <div className="flex gap-2 mb-5 flex-wrap">
                {activeCreator.campaigns.map(camp => {
                  ...
                  return (
                    <button key={camp.id} onClick={() => setSelectedCampaignId(camp.id)}
                      className="px-4 py-2 rounded-lg text-xs font-semibold cursor-pointer"
                      style={{
            ...
            {/* Empty campaigns */}
            {activeCreator.campaigns.length === 0 && (
              <div className="animate-fade-up text-center py-12 rounded-[14px]"
                style={{ background: 'var(--card)', border: '1px solid var(--border)' }}>
                <div className="text-3xl mb-3">📊</div>
                <div className="text-base font-semibold mb-1.5">No campaigns yet</div>
                <div className="text-[13px] mb-5" style={{ color: 'var(--muted)' }}>Log your first campaign to start tracking performance</div>
                <button onClick={() => setShowAddCampaign(true)}
                  className="px-6 py-2.5 rounded-lg text-[13px] font-bold cursor-pointer border-none text-white"
                  style={{ background: 'linear-gradient(135deg, #ff2d55, #c837ab)' }}>
                  + Log First Campaign
                </button>
              </div>
                ...
                <>
                  ...
                  <div className="grid grid-cols-3 gap-3.5 mb-5">
                    {[
                      ...
                      <div key={i} className="animate-fade-up rounded-[14px] p-[18px]"
                        style={{
                          background: s.accent ? 'linear-gradient(135deg, rgba(255,45,85,0.08), rgba(200,55,171,0.08))' : 'var(--card)',
                          border: s.accent ? '1px solid rgba(255,45,85,0.2)' : '1px solid var(--border)',
                  ...
                  {/* Stacked Chart */}
                  <div className="animate-fade-up rounded-[14px] p-5 mb-5"
                    style={{ background: 'var(--card)', border: '1px solid var(--border)', animationDelay: '0.2s' }}>
                    <div className="text-[10px] font-bold tracking-[1.5px] font-mono mb-3.5" style={{ color: 'var(--muted)' }}>
                      7-DAY VIEW BREAKDOWN
                    </div>
                    <StackedChart campaign={activeCampaign} />
                    <div className="flex gap-4 mt-2.5">
                      {(Object.entries(PLATFORM_CONFIG) as [PlatformKey, any][]).map(([k, v]) => (
                        <span key={k} className="flex items-center gap-1.5 text-[11px]" style={{ color: 'var(--muted)' }}>
                  ...
                  <div className="grid grid-cols-3 gap-3.5">
                    {(Object.entries(PLATFORM_CONFIG) as [PlatformKey, any][]).map(([pKey, pCfg], i) => {
                      ...
                      const dailyData = cp?.daily_views
                        ? [...cp.daily_views].sort((a, b) => a.date.localeCompare(b.date)).map(d => d.views)
                        : [];
                      while (dailyData.length < 7) dailyData.push(0);

                      return (
                        <div key={pKey} className="animate-fade-up rounded-[14px] p-[18px]"
                          style={{ background: 'var(--card)', border: '1px solid var(--border)', animationDelay: `${0.3 + i * 0.08}s` }}>
                          <div className="flex items-center justify-between mb-3.5">
                            <span className="text-xs font-bold px-2.5 py-1 rounded-md"
                              style={{ color: pCfg.color, background: pCfg.bg }}>{pCfg.icon} {pCfg.label}</span>
                            {getPlatformUsername(activeCreator, pKey) && (
                          ...
                          <div className="grid grid-cols-2 gap-x-5 gap-y-3 mb-4">
                            {[
                              { label: 'Views', val: formatNum(cp?.views || 0) },
                              { label: 'Likes', val: formatNum(cp?.likes || 0) },
                              { label: 'Comments', val: formatNum(cp?.comments || 0) },
                              { label: 'Shares', val: formatNum(cp?.shares || 0) },
                            ].map(d => (
                              <div key={d.label}>
                                <div className="text-[10px] font-semibold font-mono mb-0.5" style={{ color: 'var(--muted)' }}>{d.label}</div>
                                <div className="text-lg font-extrabold">{d.val}</div>
                              </div>
                            ))}
                          </div>
                  ...
                  {/* Verdict */}
                  <div className="animate-fade-up rounded-[14px] p-5 mt-5"
                    style={{ background: 'var(--card)', border: '1px solid var(--border)', animationDelay: '0.5s' }}>
                    <div className="text-[10px] font-bold tracking-[1.5px] font-mono mb-3" style={{ color: 'var(--muted)' }}>
                      VERDICT — IS {activeCreator.name.toUpperCase()} WORTH REBOOKING?
                    </div>
                    {(() => {
                      const bestPlat = activeCampaign.campaign_platforms
                        ?.slice().sort((a, b) => b.views - a.views)[0];
                      return (
                        <div className="flex items-center gap-5">
                          <ROIBadge cpv={campStats.cpv} />
                          <div className="text-[13px] leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
                            {formatMoney(Number(activeCampaign.spent))} spent → {formatNum(campStats.totalViews)} total views
                            = <strong style={{ color: 'var(--text)' }}>${campStats.cpv.toFixed(2)} CPM</strong>.
                            {bestPlat && (
                              <> Best platform: <strong style={{ color: PLATFORM_CONFIG[bestPlat.platform as PlatformKey]?.color }}>
                                {PLATFORM_CONFIG[bestPlat.platform as PlatformKey]?.label}
                              </strong> with {formatNum(bestPlat.views)} views.</>
                            )}
                            {campStats.cpv <= 1 ? ' Strong ROI — rebook recommended.'
                              : campStats.cpv <= 2 ? ' Decent performance — consider renegotiating rate.'
                              : ' High cost per view — explore alternatives.'}
                          </div>
                        </div>
                      );
                    })()}
      ...
      {showAddCampaign && activeCreator && (
        <AddCampaignModal creator={activeCreator} onClose={() => setShowAddCampaign(false)} onAdd={handleAddCampaign} />
      )}
    </div>
  );
}
